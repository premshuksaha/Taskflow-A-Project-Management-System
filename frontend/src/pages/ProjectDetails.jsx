...
import { ArrowLeftIcon, PlusIcon, SettingsIcon, BarChart3Icon, CalendarIcon, FileStackIcon, ZapIcon, LockIcon } from "lucide-react";
...
export default function ProjectDetail() {
    ...
    const { currentWorkspace, currentUser } = useSelector((state) => ({
        currentWorkspace: state.workspace.currentWorkspace,
        currentUser: state.workspace.currentUser
    }));
    const projects = currentWorkspace?.projects || [];
    const isAdmin = currentWorkspace?.role === 'ADMIN';
    
    // Check if analytics feature is allowed in current plan
    const isAnalyticsAllowed = currentWorkspace?.plan?.features?.includes('ANALYTICS');
            ...
            <div>
                <div className="inline-flex flex-wrap max-sm:grid grid-cols-3 gap-2 border border-zinc-200 dark:border-zinc-800 rounded overflow-hidden">
                    {[
                        ...
                        { key: "analytics", label: "Analytics", icon: BarChart3Icon, locked: !isAnalyticsAllowed },
                        ...
                        <button key={tabItem.key} onClick={() => { setActiveTab(tabItem.key); setSearchParams({ id: id, tab: tabItem.key }) }} className={`flex items-center gap-2 px-4 py-2 text-sm transition-all ${activeTab === tabItem.key ? "bg-zinc-100 dark:bg-zinc-800/80" : "hover:bg-zinc-50 dark:hover:bg-zinc-700"}`} >
                            ...
                            {tabItem.label}
                            {tabItem.locked && <LockIcon className="size-3 text-amber-500 ml-1" />}
                        </button>
                    ))}
                </div>
                    ...
                    {activeTab === "analytics" && (
                        <div className=" dark:bg-zinc-900/40 rounded max-w-6xl">
                            {isAnalyticsAllowed ? (
                                ...
                                <div className="text-center p-12 bg-zinc-50 dark:bg-zinc-900/20 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800">
                                    <LockIcon className="size-10 mx-auto mb-4 text-amber-500" />
                                    <h3 className="text-xl font-semibold mb-2">Detailed Analytics Locked</h3>
                                    <p className="text-zinc-500 dark:text-zinc-400 mb-6 max-w-md mx-auto">
                                        Advanced project insights, member productivity tracking, and milestone analysis are only available on the PRO plan.
                                    </p>
                                    <button 
