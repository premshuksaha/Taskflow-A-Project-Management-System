    ...
    const fetchUserTasks = () => {
        ...
        const currentWorkspaceTasks = currentWorkspace.projects.flatMap((project) => {
            return (project.tasks || []).filter((task) => 
                task?.assigneeId?._id === userId || task?.assigneeId === userId
            );
        });
                    ...
                    <div className="space-y-1">
                        {myTasks.length === 0 ? (
                            ...
                            myTasks.map((task, index) => (
                                <Link key={index} to={`/taskDetails?projectId=${task.projectId}&taskId=${task._id}`} className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-black dark:hover:text-white" >
