...
const ProjectTasks = ({ tasks }) => {
    ...
    const handleStatusChange = async (taskId, newStatus) => {
        try {
            ...
            await axiosInstance.put(API_PATHS.TASK.UPDATE(taskId), { status: newStatus });
    ...
    const handleDelete = async () => {
        try {
            ...
            for (const taskId of selectedTasks) {
                const task = tasks.find(t => t._id === taskId);
                await axiosInstance.delete(API_PATHS.TASK.DELETE(taskId));
                dispatch(deleteTask({ taskId, projectId: task.projectId }));
            }
                        ...
                        {filteredTasks.length > 0 ? (
                            filteredTasks.map((task) => {
                                ...
                                return (
                                    <div 
                                        key={task._id} 
                                        onClick={() => navigate(`/taskDetails?projectId=${task.projectId}&taskId=${task._id}`)}
                                        className=" dark:bg-linear-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4 flex flex-col gap-2 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                                    >
                                        <div className="flex items-center justify-between">
                                            <h3 className="text-zinc-900 dark:text-zinc-200 text-sm font-semibold">{task.title}</h3>
                                            <input onClick={e => e.stopPropagation()} type="checkbox" className="size-4 accent-zinc-600 dark:accent-zinc-500" onChange={() => selectedTasks.includes(task._id) ? setSelectedTasks(selectedTasks.filter((i) => i !== task._id)) : setSelectedTasks((prev) => [...prev, task._id])} checked={selectedTasks.includes(task._id)} />
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select onClick={e => e.stopPropagation()} name="status" onChange={(e) => handleStatusChange(task._id, e.target.value)} value={task.status} className="w-full mt-1 bg-zinc-100 dark:bg-zinc-800 ring-1 ring-zinc-300 dark:ring-zinc-700 outline-none px-2 py-1 rounded text-sm text-zinc-900 dark:text-zinc-200" >
                                                <option value="TODO">To Do</option>
                                                <option value="IN_PROGRESS">In Progress</option>
                                                <option value="DONE">Done</option>
                                            </select>
