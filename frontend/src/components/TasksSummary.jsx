...
export default function TasksSummary() {
    ...
    useEffect(() => {
        const fetchWorkspaceTasks = async () => {
            ...
            try {
                const response = await axiosInstance.get(API_PATHS.TASK.GET_BY_WORKSPACE(currentWorkspace._id));
                setTasks(response.data);
            } catch (error) {
                console.error("Tasks summary fetch error:", error);
            }
        };
    ...
    const myTasks = tasks.filter(i => i.assigneeId?._id === currentUser?._id || i.assigneeId === currentUser?._id);
                        ...
                        {card.items.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {card.items.map((issue) => (
                                    <div 
                                        key={issue._id} 
                                        onClick={() => navigate(`/taskDetails?projectId=${issue.projectId}&taskId=${issue._id}`)}
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-gray-600 dark:text-zinc-400 capitalize mt-1">
                                            {issue.type || 'task'} • {issue.priority || 'medium'} priority
                                        </p>
                                    </div>
                                ))}
                                {card.count > 3 && (
